class BudgetsController < ApplicationController 
  ...
  def find_budgets
    ...
    budgets.each do |budget|
      ...
      id = budget.id 
      lower_case_name = category.name
      name = lower_case_name.capitalize
      value = budget.value
      category_id = budget.category_id
      budget_hash = { 'id' => id, 'name' => name, 'value' => value, 'category_id' => category_id }
      budgets_arr.push(budget_hash)
      if category_ids_arr.exclude?(category_id) 
        category_ids_arr.push(category_id)
      end 
    end
    ...
    # category_ids_arr.each do |category_id| 
    #   category_sum = 0
