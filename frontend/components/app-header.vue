<template>
  <a-layout-header>
    <div class="header-col header-brand">
      <h6>
        <a-icon type="bank" theme="filled" /> <router-link to="/" class="nav-link" @click="e => e.preventDefault()">Nuxt Finance</router-link>
        <!-- <br /><span style="font-size: 13px">{{ this.$auth.loggedIn ? $auth.user.email : 'not logged in' }}</span> -->
      </h6>
    ...
    <div class="header-col header-nav">
      ...
      <a-menu mode="horizontal" class="menu-large">
        <a-menu-item>
          <router-link to="/" class="nav-link" @click="e => e.preventDefault()">
        ...
        <a-menu-item v-if="this.$auth.loggedIn">
        ...
        <a-menu-item>
          <router-link  v-if="!this.$auth.loggedIn" to="/sign-up" class="nav-link" @click="logout">
    ...
    <!-- sign up button at the edge -->
    <div v-if="!this.$auth.loggedIn" class="header-col header-btn">
      <a-button href="" target="_blank" type="primary" shape="round"><router-link to="/sign-up" class="nav-link" @click="e => e.preventDefault()">SIGN UP</router-link></a-button>
    </div>
    
    <!-- log out button at the edge -->
    <a v-if="this.$auth.loggedIn" class="ant-btn ant-btn-primary ant-btn-round log-out-button" @click.prevent="logout">LOG OUT</a>
...
.header-nav .menu-large .ant-menu-item .anticon {
  margin-right: 0;
...
.header-nav .menu-small .ant-menu-item .anticon {
  margin: 0 1px 0 3px;
  opacity: 0.5;
  font-size: 15px;
}
